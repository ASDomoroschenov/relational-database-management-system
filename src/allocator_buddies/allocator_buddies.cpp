...
short allocator_buddies::get_power_block(
...
void *allocator_buddies::get_prev_free_block(
        void *block_memory) const
{
    auto *space_is_busy = reinterpret_cast<short*>(block_memory);
    auto *space_power = reinterpret_cast<short*>(space_is_busy + 1);
...
void *allocator_buddies::get_next_free_block(
        void *block_memory) const
{
    auto *space_is_busy = reinterpret_cast<short*>(block_memory);
    auto *space_power = reinterpret_cast<short*>(space_is_busy + 1);
...
void *allocator_buddies::get_buddies(
        void *block_memory) const
{
...
bool allocator_buddies::is_free_block(
        void *block_memory) const
{
...
void allocator_buddies::deallocate(void *deallocated_block) const
{
    ...
    logger *log_memory = get_logger();
    void *current_block = reinterpret_cast<short*>(deallocated_block) - 2;

    if (log_memory != nullptr)
    {
        log_memory->log("deallocated block (" + to_string(deallocated_block) + ") : " + get_bytes_buddies(deallocated_block), logger::severity::INFORMATION);
    }
